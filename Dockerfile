...
RUN apt-get install libusb-dev gcc linux-headers-4.15.0-112-generic kmod libnuma-dev libarchive-dev libpcap-dev liblz4-dev liblz4-tool build-essential autoconf pciutils -y && \
apt-get install wget curl git vim unzip sudo python pkg-config iputils-ping -y && \
...
RUN cp /bin/uname /bin/uname.orig && \
rm -rf /bin/uname
COPY uname /bin/
